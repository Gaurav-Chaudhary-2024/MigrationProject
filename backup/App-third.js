  ...
  <div className="mb-6 p-4 bg-slate-800 rounded-lg border border-slate-700">
    ...
    <div className="mb-6">
      ...
      <p className="text-slate-300">
        This application implements a geographic-aware Markov chain model for predicting 
        foreign population redistribution across OECD countries. The model:
      </p>
      <ul className="list-disc ml-6 mt-2 text-slate-300 space-y-1">
        <li>Uses historical flow data from 2000-2020</li>
        <li>Incorporates geographic distances and border connectivity</li>
        <li>Estimates transition probabilities between countries</li>
        <li>Provides uncertainty quantification through ensemble sampling</li>
      </ul>
    </div>

    {/* Key Equations */}
    <div className="mb-6">
      <h3 className="text-lg font-semibold mb-3">Key Equations & Methodology</h3>
      
      <div className="space-y-6">
        {/* Transition Matrix Estimation */}
        <div className="bg-slate-900/50 p-4 rounded-lg">
          <h4 className="font-medium text-green-400 mb-2">1. Transition Matrix Estimation</h4>
          <div className="mb-2">
            <BlockMath math={`w_{i\\rightarrow j} = a_j e^{-\\beta \\, \\mathrm{dist}_{i,j}/1000} (1 + \\gamma \\, \\mathrm{conn}_{i,j})`} />
          </div>
          <div className="text-sm text-slate-400">
            Where:
            <ul className="ml-4 mt-1 space-y-1">
              <li>a_j: Attractiveness of destination (log of inflows)</li>
              <li>β: Distance effect parameter (0 to 1)</li>
              <li>dist_{'{i,j}'}: Haversine distance between countries</li>
              <li>γ: Connectivity effect parameter (0 to 1)</li>
              <li>conn_{'{i,j}'}: Binary border connectivity</li>
            </ul>
          </div>
        </div>
        ...
        {/* Row Normalization */}
        <div className="bg-slate-900/50 p-4 rounded-lg">
          <h4 className="font-medium text-green-400 mb-2">2. Row Normalization</h4>
          <div className="mb-2">
            <BlockMath math={`T_{i,j} = (1 - \\mathrm{stay}_i) \\frac{w_{i\\rightarrow j}}{\\sum_{k} w_{i\\rightarrow k}}, \\quad T_{i,i} = \\mathrm{stay}_i`} />
          </div>
          <div className="text-sm text-slate-400">
            Where stay_i is computed as: max(0.5, 1 - outflow/stock)
          </div>
        </div>

        {/* Population Propagation */}
        <div className="bg-slate-900/50 p-4 rounded-lg">
          <h4 className="font-medium text-green-400 mb-2">3. Population Propagation</h4>
          <div className="mb-2">
            <BlockMath math={`p_{t+1} = p_t \\, T`} />
          </div>
          <div className="text-sm text-slate-400">
            Markov chain update where p_t is the population vector at time t
          </div>
        </div>

        {/* Uncertainty Quantification */}
        <div className="bg-slate-900/50 p-4 rounded-lg">
          <h4 className="font-medium text-green-400 mb-2">4. Uncertainty Quantification</h4>
          <div className="text-sm text-slate-400">
            <ul className="space-y-1">
              <li>• Ensemble sampling with Dirichlet-like perturbations</li>
              <li>• For each row i of transition matrix:</li>
              <li className="ml-4">- Draw γ_{'{k}'} ~ Gamma(α_{'{k}'} = T_{'{i,k}'} * 100)</li>
              <li className="ml-4">- Normalize: T'_{'{i,k}'} = γ_{'{k}'} / Σ_{'{j}'} γ_{'{j}'}</li>
              <li>• Compute mean and 95% quantiles across ensemble</li>
            </ul>
          </div>
        </div>
        ...
        {/* Validation Metrics */}
        <div className="bg-slate-900/50 p-4 rounded-lg">
          <h4 className="font-medium text-green-400 mb-2">5. Validation Metrics</h4>
          <div className="space-y-2">
            <div>
              <BlockMath math="\\mathrm{RMSE} = \\sqrt{\\frac{\\sum(\\hat{y} - y)^2}{n}}" />
            </div>
            <div>
              <BlockMath math="\\mathrm{MAE} = \\frac{\\sum|\\hat{y} - y|}{n}" />
            </div>
            <div>
              <BlockMath math="\\mathrm{Coverage} = \\frac{\\text{# true values in 95\\% interval}}{\\text{total points}} \\times 100\\%" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div>
      ...
      <div className="text-sm text-slate-300 space-y-2">
        <p>• Transition matrices are estimated using geographic features and historical flows</p>
        <p>• Ensemble sampling is performed in a Web Worker for performance</p>
        <p>• Deterministic seeding available for reproducible results</p>
        <p>• Model trained on 2002-2020 data, validated on pre-2002</p>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6 overflow-y-auto">
      <div className="max-w-7xl mx-auto">
        ...
        {resultsReady && renderModelDocumentation()}
      </div>
    </div>
  );
}
